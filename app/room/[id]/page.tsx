...
export default function RoomPage() {
  ...
  useEffect(() => {
    ...
    const manager = new WebRTCManager(
      ...
      (file: MediaFile) => {
        const senderName =
          file.sender === clientId
            ? "You"
            : (() => {
                const participant = participants.find((p) => p.clientId === file.sender)
                return getDisplayName(participant, file.sender)
              })()
      ...
      (updatedParticipants: Participant[]) => {
        ...
        const validParticipants = updatedParticipants.filter((p) => p && p.clientId && typeof p.clientId === "string")
    ...
    const handleReceiveMessage = (data: any) => {
      console.log("[v0] Received message:", data.message)
      const message: Message = {
        id: crypto.randomUUID(),
        text: data.message,
  ...
  }, [roomId, clientId, displayName]) // Removed participants from dependencies
  ...
  const getDisplayName = (participant: Participant | undefined, clientId: string): string => {
    if (!participant || !participant.displayName) {
      const safeClientId = clientId && typeof clientId === "string" ? clientId : "Unknown"
      return `User ${safeClientId.slice(0, 8)}`
    }
      ...
      <header className="bg-[#4B2E2E] text-white p-2 sm:p-4 flex-shrink-0">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 sm:gap-2 ml-2">
            ...
            <Button
              variant="outline"
              className="border-white text-white hover:bg-white hover:text-[#4B2E2E] bg-transparent text-xs h-8 px-2"
                  ...
                  <div className="flex gap-1 ml-auto flex-wrap">
                    {participants.slice(0, 3).map((participant) => (
                      <Badge key={participant.clientId} variant="secondary" className="text-xs">
                        {getDisplayName(participant, participant.clientId).slice(0, 8)}
                      </Badge>
                    ))}
                  </div>
                </CardTitle>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4 h-full">
                  <div className="bg-gray-100 rounded-lg flex items-center justify-center border-2 border-gray-200 relative min-h-[200px] sm:min-h-[250px]">
                    ...
                    <div className="absolute bottom-2 left-2 bg-black bg-opacity-50 text-white px-2 py-1 rounded text-xs">
                      {displayName ? displayName.slice(0, 10) : "You"}
                    </div>
                  </div>
                    ...
                    participants.slice(0, 3).map((participant) => (
                      ...
                      >
                        {participant.stream ? (
                          ...
                          <div className="text-center text-gray-500">
                            <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gray-300 rounded-full mx-auto mb-2"></div>
                            <p className="text-sm">{getDisplayName(participant, participant.clientId).slice(0, 10)}</p>
                            <p className="text-xs">(No video)</p>
                          </div>
                        )}
                        <div className="absolute bottom-2 left-2 bg-black bg-opacity-50 text-white px-2 py-1 rounded text-xs">
                          {getDisplayName(participant, participant.clientId).slice(0, 10)}
                        </div>
                      </div>
                    ))
                  ) : (
